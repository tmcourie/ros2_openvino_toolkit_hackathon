...
/**
 * @brief Source file for YOLO v7 model
 * @file object_detection_yolov7_model.cpp
 */
#include <string>
#include <memory>
#include <vector>
...
Models::ObjectDetectionYolov7Model::ObjectDetectionYolov7Model(
    const std::string &label_loc, const std::string &model_loc, int max_batch_size)
    : ObjectDetectionModel(label_loc, model_loc, max_batch_size)
{
}

bool Models::ObjectDetectionYolov7Model::updateLayerProperty(
    std::shared_ptr<ov::Model> &model)
{
  ...
  input_info.tensor().set_element_type(ov::element::u8).set_layout(input_tensor_layout).set_color_format(ov::preprocess::ColorFormat::BGR);
...
const std::string Models::ObjectDetectionYolov7Model::getModelCategory() const
{
  return "Object Detection Yolo v7";
}

bool Models::ObjectDetectionYolov7Model::enqueue(
...
bool Models::ObjectDetectionYolov7Model::matToBlob(
...
bool Models::ObjectDetectionYolov7Model::fetchResults(
    const std::shared_ptr<Engines::Engine> &engine,
    ...
    const bool &enable_roi_constraint)
{
  ...
  for (int idx : nms_result)
  {
    ...
    std::string label = class_ids[idx] < labels.size() ? labels[class_ids[idx]] : std::string("label #") + std::to_string(class_ids[idx]);
...
Models::Resize_t Models::ObjectDetectionYolov7Model::pre_process_ov(const cv::Mat &input_image)
{
